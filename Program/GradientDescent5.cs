...
public static class GradientDescent5
{
	...
	public static void Solve()
	{
		...
		{
			x=> x[0]        + x[1]*x[1]*3 - x[2]      - 0.12,
			x=> x[0]*x[0]*3 + x[1]        + x[2]*x[2] - 4.2,
			x=> x[0]*4      + x[1]        - x[2]*x[2] - 3.2,
		};
		...
		var r2 = fnVector.Apply(new double[] { 1, 0.2, 1 });
		Console.Out.WriteVector(r2);
		Console.Out.WriteLine();

		var r3 = fnVector.Apply(new double[] { 1.058, -0.0952, 0.9676 });
		Console.Out.WriteVector(r3);
		Console.Out.WriteLine();
	}
